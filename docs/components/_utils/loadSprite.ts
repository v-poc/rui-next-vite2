/*
 * Inspired by svg-sprite-loader | MIT License (https://github.com/JetBrains/svg-sprite-loader)
 * Please make sure run `loadSprite` after document ready.
 */
const SVG_SPRITE_NODE_ID = "__RUI_SVG_SPRITE_NODE__";
...
const renderSvgSprite = () => {
  const symbols = Object.keys(defaultIcons)
    .map(iconName => {
      const svgContent = defaultIcons[iconName].split("svg")[1];
...
const loadSprite = () => {
  ...
  const svgSpriteNode = document.getElementById(SVG_SPRITE_NODE_ID);
  if (!svgSpriteNode) {
